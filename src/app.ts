...
const injectVsbilHead = (html: string) => { if (html.includes('rel="manifest"')) return html; const head = `\n<link rel="manifest" href="/manifest.webmanifest">\n<link rel="icon" href="/assets/vsbil-logo.svg" type="image/svg+xml">\n<link rel="apple-touch-icon" href="/assets/vsbil-logo.svg">\n<meta name="apple-mobile-web-app-capable" content="yes">\n<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">\n<meta name="apple-mobile-web-app-title" content="VSBIL">\n<link rel="stylesheet" href="/css/brand.css">\n`; const script = `<script src="/js/pwa.js" defer></script>`; return html.replace("</head>", `${head}</head>`).replace("</body>", `${script}</body>`); };
const sendHtmlPage = async (filePath: string, res: express.Response, next: express.NextFunction) => { try { const html = await readFile(filePath, "utf8"); res.status(200).type("html").setHeader("X-Content-Type-Options", "nosniff"); res.send(injectVsbilHead(html)); } catch (error: any) { if (error?.code === "ENOENT") return next(); return next(error); } };
...
app.use((error: unknown, _req: express.Request, res: express.Response, _next: express.NextFunction) => { console.error("Unhandled application error", error); if (res.headersSent) return; res.status(500).json({ success: false, message: "Internal server error", code: "INTERNAL_ERROR", requestId: res.getHeader("X-Request-Id") }); });
export default app;
