...
app.use(cors({ origin: (origin, cb) => { if (!origin) return cb(null, true); const normalized = normalizeOrigin(origin); if (!isProduction) return cb(null, true); return productionOrigins.has(normalized) ? cb(null, true) : cb(new Error("CORS origin denied")); }, credentials: true, methods: ["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"], allowedHeaders: ["Content-Type", "Authorization", "Accept", "X-Requested-With", "X-Idempotency-Key"], exposedHeaders: ["X-Request-Id"], optionsSuccessStatus: 204 }));
