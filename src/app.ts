...
app.use(cors({ origin: (origin, cb) => { if (!origin) return cb(null, true); const normalized = origin.replace(/\/$/, ""); if (allowedOrigin && normalized === allowedOrigin) return cb(null, true); if (!isProduction && (normalized.startsWith("http://localhost:") || normalized.startsWith("http://127.0.0.1:") || normalized.endsWith(".github.dev"))) return cb(null, true); return cb(new Error("CORS origin denied")); }, credentials: true, methods: ["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"], allowedHeaders: ["Content-Type", "Authorization", "Accept", "X-Requested-With", "X-Idempotency-Key"], exposedHeaders: ["X-Request-Id"], optionsSuccessStatus: 204 }));
...
const sendHtmlPage = async (filePath: string, res: express.Response, next: express.NextFunction) => {
  try { const html = await readFile(filePath, "utf8"); res.status(200).type("html").setHeader("X-Content-Type-Options", "nosniff").setHeader("Cache-Control", isProduction ? "public, max-age=60, must-revalidate" : "no-store").send(injectVsbilHead(html)); }
...
app.use((error: unknown, _req: express.Request, res: express.Response, _next: express.NextFunction) => { console.error("Unhandled application error", error); if (res.headersSent) return; const isCors = error instanceof Error && error.message === "CORS origin denied"; res.status(isCors ? 403 : 500).json({ success: false, message: isCors ? "Origin is not allowed" : "Internal server error", code: isCors ? "CORS_DENIED" : "INTERNAL_ERROR", requestId: res.getHeader("X-Request-Id") }); });
export default app;
