...
const nodeEnv=process.env.NODE_ENV?.trim().toLowerCase()||"development",isProduction=nodeEnv==="production";
const configuredOrigins=new Set((process.env.APP_URL??"").split(",").map(v=>v.trim().replace(/\/$/,"")).filter(Boolean));
...
app.use(cors({origin:(origin,cb)=>{if(!origin||!isProduction)return cb(null,true);const n=origin.replace(/\/$/,"");return configuredOrigins.has(n)?cb(null,true):cb(new Error("CORS origin denied"))},credentials:true,methods:["GET","POST","PUT","PATCH","DELETE","OPTIONS"],allowedHeaders:["Content-Type","Authorization","Accept","X-Requested-With","X-Idempotency-Key"],exposedHeaders:["X-Request-Id"],optionsSuccessStatus:204}));
...
app.use("/api/support",rateLimit({windowMs:60000,max:8,key:req=>`${req.ip}:support`}),supportRouter);
app.use("/api/dashboard",dashboardRouter);app.use("/api/withdrawals",withdrawalRouter);app.use("/api/activities",rateLimit({windowMs:60000,max:60,key:req=>`${req.ip}:activities`}),activityRouter);app.use("/api/users",usersRouter);app.use("/api/notifications",notificationRouter);app.use("/api/admin",supportAdminRouter);app.use("/api/admin",campaignAdminRouter);app.use("/api/admin",adminRouter);
...
app.use(express.static(publicDirectory,{extensions:["html"],setHeaders:r=>r.setHeader("X-Content-Type-Options","nosniff")}));
