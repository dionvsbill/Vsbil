...
const send = async (file: string, res: express.Response, next: express.NextFunction) => { try { const html = await readFile(file, "utf8"); res.status(200).type("html").setHeader("X-Content-Type-Options", "nosniff").setHeader("Cache-Control", isProduction ? "public,max-age=60,must-revalidate" : "no-store").send(inject(html)); } catch (error: any) { if (error?.code === "ENOENT") return next(); return next(error); } };
