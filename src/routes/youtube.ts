...
function readState(value: string): string {
  ...
  if (!userId || !Number.isFinite(issuedAt) || Date.now() - issuedAt < 0 || Date.now() - issuedAt > stateLifetimeMs) throw new Error("EXPIRED_STATE");
  return userId;
}
...
router.get("/callback", async (req: Request, res: Response) => {
  try {
    ...
    if (oauthError) return res.redirect(`${env("APP_URL")}/settings.html?youtube=cancelled`);
    ...
    const [channelResponse, userInfoResponse] = await Promise.all([
      fetch("https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&mine=true", { headers: { Authorization: `Bearer ${tokens.access_token}` } }),
    ...
    const googleUser: any = await userInfoResponse.json().catch(() => null);
    const channel = channels?.items?.[0];
    if (!channel?.id) return res.status(400).send("No YouTube channel was found on this Google account.");

    const now = new Date().toISOString();
    const encryptedAccessToken = encryptSecret(String(tokens.access_token));
    const encryptedRefreshToken = tokens.refresh_token ? encryptSecret(String(tokens.refresh_token)) : undefined;
    const row: Record<string, unknown> = {
      user_id: userId,
      google_subject: typeof googleUser?.sub === "string" ? googleUser.sub : null,
      google_email: typeof googleUser?.email === "string" ? googleUser.email : null,
    ...
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (!row.refresh_token_encrypted && existing?.refresh_token_encrypted) row.refresh_token_encrypted = existing.refresh_token_encrypted;
...
router.get("/connection", requireIdentity, async (req, res) => {
  const { data, error } = await supabase
    .from("youtube_connections")
    .select("channel_id,channel_title,channel_url,channel_thumbnail_url,status,connected_at,last_verified_at,last_sync_at,updated_at")
...
router.delete("/connection", requireIdentity, async (req, res) => {
  const { data: connection } = await supabase.from("youtube_connections").select("id,channel_id").eq("user_id", req.user!.id).eq("status", "connected").maybeSingle();
  if (!connection) return res.json({ success: true });
  const { error } = await supabase.from("youtube_connections").update({ status: "disconnected", access_token_encrypted: null, refresh_token_encrypted: null, updated_at: new Date().toISOString() }).eq("id", connection.id);
  if (error) return res.status(500).json({ success: false, message: "Unable to disconnect YouTube" });
  await supabase.from("security_events").insert({ user_id: req.user!.id, event_type: "youtube_disconnected", severity: "info", metadata: { channel_id: connection.channel_id } });
...
router.post("/verify-subscription", requireIdentity, async (req, res) => {
  ...
  const { data: connection } = await supabase
    .from("youtube_connections")
    .select("id,channel_id,access_token_encrypted,refresh_token_encrypted,status")
    .eq("user_id", req.user!.id)
    .eq("status", "connected")
    .order("connected_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  if (!connection) return res.status(403).json({ success: false, message: "Connect your YouTube account before using subscription verification", code: "YOUTUBE_NOT_CONNECTED" });

  try {
    let accessToken = connection.access_token_encrypted ? decryptSecret(connection.access_token_encrypted) : "";
    let tokenExpiresSoon = true;
    if (accessToken) {
      const probe = await fetch("https://www.googleapis.com/youtube/v3/channels?part=id&mine=true", { headers: { Authorization: `Bearer ${accessToken}` } });
      tokenExpiresSoon = !probe.ok;
    }
    if (tokenExpiresSoon) {
      if (!connection.refresh_token_encrypted) throw new Error("TOKEN_REFRESH_UNAVAILABLE");
      ...
      await supabase.from("youtube_connections").update({ access_token_encrypted: encryptSecret(accessToken), status: "connected", updated_at: new Date().toISOString() }).eq("id", connection.id);
    }
    ...
    const check = await fetch(url, { headers: { Authorization: `Bearer ${accessToken}` } });
    ...
    await supabase.from("youtube_connections").update({ last_verified_at: checkedAt, last_sync_at: checkedAt, status: "connected", updated_at: checkedAt }).eq("id", connection.id);
    await supabase.from("youtube_verification_events").insert({ user_id: req.user!.id, youtube_connection_id: connection.id, verification_type: "subscription", result: subscribed ? "verified" : "not_verified", evidence: { target_channel_id: targetChannelId, checked_at: checkedAt, method: "youtube_api" } });
    return res.json({ success: true, subscribed, checkedAt, verification: "youtube_api" });
  } catch (error) {
    console.error("Subscription verification", error);
    await supabase.from("youtube_verification_events").insert({ user_id: req.user!.id, youtube_connection_id: connection.id, verification_type: "subscription", result: "error", evidence: { target_channel_id: targetChannelId } });
