...
(function () {
  ...
  root.innerHTML = `
    <header class="vs-topbar">
      ...
      <div class="vs-actions">
        <a class="vs-icon" href="/discover.html" aria-label="Discover">⌕</a>
    ...
    <div class="vs-layout">
      <aside class="vs-sidebar" id="vsSidebar">
        <nav class="vs-nav" aria-label="VSBIL navigation">
          ...
          <a data-nav="youtube" href="/youtube.html"><b>◉</b><span>YouTube</span></a>
    ...
    <nav class="vs-bottom" aria-label="Mobile navigation">
      <a data-nav="home" href="/"><b>⌂</b><span>Home</span></a>
  ...
  if (token && window.VSBIL_AUTH?.api) {
    window.VSBIL_AUTH.api("/api/users/profile").then((d) => {
      const u = d.user || {};
      const avatar = document.getElementById("vsAvatar");
      if (!avatar) return;
      avatar.innerHTML = u.avatar_url ? `<img src="${String(u.avatar_url).replace(/\"/g, "&quot;")}" alt="Profile">` : String(u.username || "U").slice(0, 1).toUpperCase();
    }).catch(() => {});
  }
  const search = document.getElementById("vsSearch");
  if (search) search.addEventListener("keydown", (e) => { if (e.key === "Enter" && search.value.trim()) location.href = `/discover.html?q=${encodeURIComponent(search.value.trim())}`; });
})();
