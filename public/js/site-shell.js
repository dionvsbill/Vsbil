(() => {
  ...
  async function homeSession() {
    ...
    document.querySelectorAll('a[href="/register.html"],a[href="/login.html"]').forEach(a => {
      if (/create account|login/i.test(a.textContent || "")) {
  ...
  function loader() {
    ...
    const hide = () => requestAnimationFrame(() => setTimeout(() => e.classList.add("is-hidden"), 850));
    addEventListener("load", hide, { once: true });
    setTimeout(hide, 1500);
  }

  function navigation() {
    // Login/register remain clean. Every other public or authenticated page gets the same shell.
    ...
    const links = [
      ["⌂", "Home", "/"],
      ["◎", "For You", "/discover.html"],
      ["▶", "YouTube", "/youtube.html"],
      ["◈", "Campaigns", "/creator.html"],
      ["◇", "Shop", "/shop.html"],
    ...
    aside.id = "vsbilGlobalNavigation";
    aside.className = "vsbil-global-sidebar";
    aside.innerHTML = `<a class="vsbil-global-nav-brand" href="/" aria-label="VSBIL home"><img src="${LOGO}" alt="VSBIL"><span>VSBIL</span></a><nav aria-label="VSBIL primary navigation">${nav}</nav><div class="vsbil-sidebar-account">${hasSessionToken() ? `<a href="/profile.html"><b>●</b><span>Profile</span></a><a href="/settings.html"><b>⚙</b><span>Settings</span></a>` : `<a href="/login.html"><b>→</b><span>Sign in</span></a><a href="/register.html"><b>＋</b><span>Create account</span></a>`}</div>`;
    document.body.prepend(aside);

    const bottom = document.createElement("nav");
    bottom.id = "vsbilGlobalBottomNav";
    bottom.className = "vsbil-global-bottom-nav";
    bottom.setAttribute("aria-label", "VSBIL mobile navigation");
  ...
  function footer(isAuthenticated) {
    ...
    const accountLinks = isAuthenticated
      ...
      : `<a href="/login.html">Login</a><a href="/register.html">Create Account</a><a href="/dashboard.html">Dashboard</a>`;
  ...
  function boot() {
    // Render the shell immediately; session checks must never block navigation rendering.
    if (!isAuthPage()) {
      loader();
      navigation();
      footer(hasSessionToken());
      homeSession();
    } else {
