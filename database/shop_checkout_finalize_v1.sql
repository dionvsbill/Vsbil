...
begin
  ...
  select * into created_row from public.create_shop_order(s.shop_id,s.buyer_name,s.buyer_phone,s.buyer_email,s.delivery_address,s.delivery_note,s.payment_method,(select delivery_fee from public.business_shops where id=s.shop_id),s.items);
  fee:=round(created_row.total*public.shop_fee_percent(s.shop_id)/100,2);
  net:=greatest(0,created_row.total-fee);
  update public.shop_orders set payment_status='paid',payment_reference=s.reference,escrow_status='held',platform_fee_amount=fee,seller_pending_amount=net,fee_percent=public.shop_fee_percent(s.shop_id),updated_at=now() where id=created_row.order_id returning * into result_row;
  ...
  insert into public.shop_wallet_holds(shop_id,order_id,gross_amount,fee_amount,net_amount,status) values(result_row.shop_id,result_row.id,result_row.total,fee,net,'held') on conflict(order_id) do nothing;
  return result_row;
end; $$;
